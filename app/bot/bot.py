...
class BotHandler:
    ...
    async def debounce_and_send(self, chat_id):
    ...
    async def send_reply(self, chat_id):
        reply = await open_ai_client.generate_response(chat_id)
        messages = json.loads(reply)["messages"]
        for message in messages:
            async with self.client.action(chat_id, 'typing'):
                await asyncio.sleep(len(message / random.uniform(2, 8)))
    ...
    async def handle_message(self, event):
        ...
        state.debounce_task = asyncio.create_task(self.debounce_and_send(chat_id))
